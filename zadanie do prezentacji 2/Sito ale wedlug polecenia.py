...
def sito(n):
    ...
    for i in range(2, int(math.ceil(math.sqrt(n)))):    #sprawdzanie do pierwiastka danej liczby jest optymalne, bo nie wyjdziemy poza zakres "n"
        if liczby[i]:
...
print("Program metodą sita Eratostenesa wyznaczy liczby pierwsze mniejsze od n (gdzie n to liczba wproawdzona przez użytkownika).")
run=True
while run:
